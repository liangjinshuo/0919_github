 ...
 */
type Result struct {
	// ResultID string `json:"result_id"`11
}
/*Handler 函数入口
...
func Handler(ctx context.Context, params *Params) (*Result, error) {
	...
	// 在这里补充业务代码
        application.GetLogger(ctx).Infof("sleep....")
	time.Sleep(190 * time.Minute)
        application.GetLogger(ctx).Infof("%s 函数执行结束", time.Now().Format("2006-01-02 15:04:05.999"))
	return &Result{}, nil
}
